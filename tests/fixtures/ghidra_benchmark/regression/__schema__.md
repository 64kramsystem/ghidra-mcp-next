...
stored under `tests/fixtures/ghidra_benchmark/build/`. Every file here is consumed
by `tools.setup.ghidra.run_benchmark_yaml_regression` during
`python -m tools.setup deploy --test release`.
...
```
tests/fixtures/ghidra_benchmark/regression/
...
1. Build and import the fixture with the release regression workflow.
2. Review assertion changes against the C source and endpoint contracts.
