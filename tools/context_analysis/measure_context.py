#!/usr/bin/env python3
"""
Measure context-window usage for ghidra-mcp's current MCP tool catalog.
...
def analyze_endpoints():
    ...
    if HAS_TIKTOKEN:
        ...
        print(f"  Avg per endpoint:      {tokens // endpoint_count:>10} tokens")
    else:
        print("TOKEN ESTIMATES (character-based, ÷4 approximation):")
        print(f"  Full endpoints.json:   ~{tokens:>9,} tokens (rough)")
        print(f"  Avg per endpoint:      ~{tokens // endpoint_count:>9} tokens (rough)")
    print()
    ...
    print(f"   - All {endpoint_count} endpoints use ~{tokens:,} tokens")
    ...
    print("   - Lazy tool groups loaded through load_tool_group")
