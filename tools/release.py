...
Only `v<semver>` counts — this repository's `build-*` tags are not releases.
...
builds and inspects the release candidate artifacts, commits, tags, pushes, and
...
DEFAULT_BRANCH = "main"
# Under dist/: ignored by git, and excluded from the packaged extension.
...
def head_release_tag(repo_root: Path, runner: Runner = run) -> str | None:
    ...
    Only `v<semver>` counts: this repository also carries `build-*` tags, which
    are not releases.
    """
    for tag in runner(
...
def origin_repo(repo_root: Path, runner: Runner = run) -> str:
    ...
    # Backslashes normalised so a Windows path-style remote parses too: the test
    # fixtures use a local bare repo as origin, and CI runs them on Windows.
...
def verify_manifest(
    ...
    The manifest is untrusted input: it is a file on disk that anything could
    have written. So its artifact set is compared against the canonical set for
    this version *first* — otherwise `{"artifacts": []}` would publish a lone
    checksum file and an "exact asset set" check derived from the same manifest
    would happily agree.

    Hashes are then checked, because an artifact rebuilt from a different commit
...
def _rollback(
    ...
    if release_commit is not None:
        ...
        if current == release_commit:
            ...
            # so leave the release commit in place and say so.
            raise ReleaseError(
