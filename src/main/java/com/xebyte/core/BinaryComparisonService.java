 ...
 * functions across binaries compiled by different compilers. Endpoint methods
 * delegate to static, stateless, thread-safe comparison algorithms.
 *
...
public class BinaryComparisonService {
     ...
     */
    @McpTool(path = "/get_function_hash", description = "Compute normalized opcode hash for function. On programs with multiple address spaces (e.g., embedded targets), prefix addresses with the space name (mem:1000) to avoid ambiguous resolution.", category = "comparison")
     ...
     */
    @McpTool(path = "/get_bulk_function_hashes", description = "Get hashes for multiple or all functions", category = "comparison")
     ...
     */
    @McpTool(path = "/get_function_signature", description = "Get function signature for cross-binary comparison. On programs with multiple address spaces (e.g., embedded targets), prefix addresses with the space name (mem:1000) to avoid ambiguous resolution.", category = "comparison")
    public Response getFunctionSignature(
            @Param(value = "address", paramType = "address",
                   description = "Address in the program. Accepts 0x<hex> (default space) or <space>:<hex> "
                               + "(e.g., mem:1000, code:ff00). Note: some programs — particularly "
                               + "embedded/microcontroller targets — are not address-space-agnostic; "
        ...
        try {
            ...
            FunctionSignature sig =
                computeFunctionSignature(program, func, new ConsoleTaskMonitor());
     ...
     */
    @McpTool(path = "/find_similar_functions_fuzzy", description = "Cross-binary fuzzy function matching. On programs with multiple address spaces (e.g., embedded targets), prefix addresses with the space name (mem:1000) to avoid ambiguous resolution.", category = "comparison")
    public Response findSimilarFunctionsFuzzy(
            @Param(value = "address", paramType = "address",
                   description = "Address in the program. Accepts 0x<hex> (default space) or <space>:<hex> "
                               + "(e.g., mem:1000, code:ff00). Note: some programs — particularly "
                               + "embedded/microcontroller targets — are not address-space-agnostic; "
        ...
        try {
            ...
            return findSimilarFunctionsJson(
     ...
     */
    @McpTool(path = "/bulk_fuzzy_match", description = "Bulk cross-binary function matching", category = "comparison")
    public Response bulkFuzzyMatch(
            @Param(value = "source_program", description = "Source program name") String sourceProgramName,
            @Param(value = "target_program", description = "Target program name") String targetProgramName,
            @Param(value = "threshold", defaultValue = "0.7", description = "Similarity threshold") double threshold,
            @Param(value = "offset", defaultValue = "0") int offset,
     ...
     */
    @McpTool(path = "/diff_functions", description = "Compute structured diff between two functions. On programs with multiple address spaces (e.g., embedded targets), prefix addresses with the space name (mem:1000) to avoid ambiguous resolution.", category = "comparison")
    public Response diffFunctions(
        ...
        try {
            ...
            return diffFunctionsJson(progA, funcA, progB, funcB, new ConsoleTaskMonitor());
        } catch (Exception e) {
            return Response.err(e.getMessage());
        }
    }
